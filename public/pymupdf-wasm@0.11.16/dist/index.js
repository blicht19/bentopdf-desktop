...
`);
  ...
  findTables(options) {
    ...
    if (options?.addLines && options.addLines.length > 0) {
      const linesStr = options.addLines
        .map((l) => `(${l.join(',')})`)
        .join(',');
...
`);
  ...
  getPage(index) {
    ...
    if (index < 0 || index >= this.pageCount) {
      throw new Error(
        `Page index ${index} out of range (0-${this.pageCount - 1})`
      );
    }
    return new PyMuPDFPage((code) => this.runPython(code), this.docVar, index);
  }
  *pages() {
    this.ensureOpen();
  ...
  insertBlankPage(index, width, height) {
    ...
    const h = height ?? 842;
    this.runPython(
      `${this.docVar}.insert_page(${index}, width=${w}, height=${h})`
    );
...
async function convertPdfToRgb(pdfData, gsBaseUrl) {
  if (!gsBaseUrl) {
    throw new Error(
      'Ghostscript URL not configured. Cannot perform RGB conversion.'
    );
  }
  ...
  console.log(
    '[convertPdfToRgb] Running Ghostscript with args:',
    args.join(' ')
  );
  let exitCode;
  try {
  ...
  if (exitCode !== 0) {
    ...
    throw new Error(
      `Ghostscript RGB conversion failed with exit code ${exitCode}`
    );
  }
  let output;
  try {
  ...
  copy.set(output);
  console.log(
    '[convertPdfToRgb] Conversion complete, output size:',
    copy.length
  );
  return copy;
}
...
var PyMuPDF = class {
  ...
  async initPyodide() {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch pyodide.js: HTTP ${response.status}`);
    }
    ...
    let pyodideModule;
    try {
      pyodideModule = await import(/* @vite-ignore */ blobUrl);
    } finally {
    ...
    await Promise.all(
      ASSETS.wheels.map((wheel) =>
        pyodide.loadPackage(this.getAssetPath(wheel))
      )
    );
    pyodide.runPython(`
import pymupdf
...
`);
  ...
  async pdfToDocx(pdf, pages) {
    ...
    console.log(
      '[pdfToDocx] Converting PDF to RGB colorspace with Ghostscript...'
    );
    try {
      ...
      console.warn(
        '[pdfToDocx] Ghostscript RGB conversion failed, trying original:',
        e
      );
    }
...
`);
  ...
  async convertToPdf(file, options) {
    ...
    const ext =
      options?.filetype ?? filename.split('.').pop()?.toLowerCase() ?? '';
...
`);
  ...
  async pdfToImages(pdf, options) {
    ...
    const pagesToExport =
      options?.pages ?? Array.from({ length: pageCount }, (_, i) => i);
    const results = [];
...
`);
  ...
  async htmlToPdf(html, options) {
    ...
    const cssBase64 = options?.css
      ? uint8ArrayToBase642(encoder.encode(options.css))
      : '';
...
export { PyMuPDF, PyMuPDFDocument, PyMuPDFPage };
